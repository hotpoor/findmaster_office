...
class DataWebSocket(WebSocket):
    clients = set()
    h_clients = {}
    free_room_ids = ["APIDEMO","MSH_USER","OFFICE","HOLLOW_USER"]
    @staticmethod
            ...
            if info[0] == "JOINMOREROOMS":
                for aim_id in info[3]:
                    if not (aim_id in DataWebSocket.free_room_ids):
                        if not aim_id.startswith("MSH_USER") or not aim_id.startswith("HOLLOW_USER"):
                            return
                    h_clients_base = set()
        ...
        if len(info) == 3:
            for c in DataWebSocket.h_clients.get(info[2],[]):
                if c == self:
                    ...
                    if info[0]!="LEAVE":
                        if info[0] in ["JOIN"]:
                            to_self_message = json_encode([info[0],{"content":"SELF","num":info[1]["num"]}])
    ...
    def open(self):
        ...
        num = len(h_clients_base)
        msg = ["JOIN",{"content":str(id(self)) + 'has joined room:'+self.aim_id,"num":num},self.aim_id]
    ...
    def on_close(self):
        ...
        for aim_id in self.aim_ids:
            ...
            if len(h_clients_base) == 0:
                ...
                msg = ["LEAVE",{"content":str(id(self)) + 'last one has left room:'+aim_id,"num":0},aim_id]
                print(len(h_clients_base),msg)
            else:
                DataWebSocket.h_clients[aim_id] = h_clients_base
                msg = ["LEAVE",{"content":str(id(self)) + 'has left room:'+aim_id,"num":len(h_clients_base)},aim_id]
                print(len(h_clients_base),msg)
                msg = json_encode(msg);
                DataWebSocket.send_to_other(self,msg)
