...
class GroundHandler(WebRequest):
    @tornado.gen.coroutine
    def get(self):
        entity_id = settings.get("store_ground",None)
        block_id = entity_id
        ...
        self.render("../template/store/ground.html")
class HollowHandler(WebRequest):
    @tornado.gen.coroutine
    def get(self):
        entity_id = settings.get("store_hollow",None)
        block_id = entity_id
        MPWeixinAPP = self.get_argument('app',settings["MPWeixinAppDefault"])
        is_weixin = False
        if "Mobile" in self.request.headers.get("User-Agent", "") and "MicroMessenger" in self.request.headers.get("User-Agent", ""):
        ...
        self.ref_id = self.get_argument("ref_id",None)
        # self.render("../template/msh_hollow.html")
        self.render("../template/store/hollow.html")

class SearchHandler(WebRequest):
    @tornado.gen.coroutine
    def get(self):
        entity_id = settings.get("store_search",None)
        block_id = entity_id
        MPWeixinAPP = self.get_argument('app',settings["MPWeixinAppDefault"])
        is_weixin = False
        if "Mobile" in self.request.headers.get("User-Agent", "") and "MicroMessenger" in self.request.headers.get("User-Agent", ""):
        ...
        self.ref_id = self.get_argument("ref_id",None)
        # self.render("../template/msh_hollow.html")
        self.render("../template/store/search.html")
class UserEntityHandler(WebRequest):
    @tornado.gen.coroutine
    def get(self,entity_id):
        ...
        self.ref_id = self.get_argument("ref_id",None)
        # self.render("../template/msh_hollow.html")
        self.render("../template/store/user.html")

class ShopEntityHandler(WebRequest):
    @tornado.gen.coroutine
    def get(self,entity_id):
        entity_id = entity_id
        block_id = entity_id
        ...
        self.ref_id = self.get_argument("ref_id",None)
        # self.render("../template/msh_hollow.html")
        self.render("../template/store/shop.html")

class ProductEntityHandler(WebRequest):
    @tornado.gen.coroutine
    def get(self,store_id,entity_id):
        entity_id = entity_id
        block_id = entity_id
        MPWeixinAPP = self.get_argument('app',settings["MPWeixinAppDefault"])
        is_weixin = False
        ...
        self.render("../template/store/product.html")
class OrderEntityHandler(WebRequest):
    @tornado.gen.coroutine
    def get(self,entity_id):
        entity_id = entity_id
        block_id = entity_id
        ...
        self.ref_id = self.get_argument("ref_id",None)
        # self.render("../template/msh_hollow.html")
        self.render("../template/store/order.html")
class HollowEntityHandler(WebRequest):
    @tornado.gen.coroutine
    def get(self,entity_id):
        entity_id = entity_id
        block_id = entity_id
...
class AddStoreAPIHandler(WebRequest):
    def post(self):
        if not self.current_user:
        ...
        if user_id not in editors:
            ...
            return
        if block_id in stores:
            self.finish({"info":"error","about":"already has store"})
            return
        stores.insert(0,block_id)
        user["stores"]=stores
        user["updatetime"]=int(time.time())
        update_aim(user_id,user)
        self.finish({"info":"ok","about":"add a store success","block_id":block_id})
class InfoStoreAPIHandler(WebRequest):
    @tornado.gen.coroutine
    def post(self):
        store_ids = self.get_argument("store_ids","[]")
        store_ids = json_decode(store_ids)
        store_ids = list(set(store_ids))
        stores = get_aims(store_ids)
        result_list=[]
        for store in stores:
            result_list_item = {
                "store_id":store[0],
                "hollows":store[1].get("hollows",[]),
                "title":store[1].get("title","A New Store"),
                "desc":store[1].get("desc","Winni app has a new store"),
                "headimgurl":store[1].get("headimgurl","/static/img/msh_icon/home.png"),
                "store_score":store[1].get("store_score","5.0"),
            }
            result_list.append(result_list_item)
        result_list = json_encode(result_list)
        result_list = json_decode(result_list)
        self.finish({"info":"ok","about":"success","result":result_list})
class ListStoreAPIHandler(WebRequest):
    def get(self):
        user_id = self.get_argument("user_id",None)
        user = get_aim(user_id)
        if not user:
            self.finish({"info":"error","about":"no user"})
            return
        store_ids = user.get("stores",[])
        stores = get_aims(store_ids)
        result_list = []
        for store in stores:
            result_list_item = {
            ...
            result_list.append(result_list_item)
        self.finish({"info":"ok","about":"result stores info","result":result_list})
class ChangeStoreAPIHandler(WebRequest):
    def post(self):
        pass
class OpenStoreAPIHandler(WebRequest):
    def post(self):
        pass
class CloseStoreAPIHandler(WebRequest):
    def post(self):
        pass
