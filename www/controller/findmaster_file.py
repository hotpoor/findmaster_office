...
class ImageCheckAPIHandler(WebRequest):
    ...
    def post(self):
        ...
        if not block or not md5:
            ...
            return
        if subtype not in ["msh_hollow","page","hollow","shop","product"]:
        ...
        uptoken = q.upload_token("office-xialiwei")
        self.finish({
            "token": uptoken,
            "exists": False
        })
...
class FileCheckAPIHandler(WebRequest):
    @tornado.gen.coroutine
    def post(self):
        self.set_header("Access-Control-Allow-Origin", self.request.headers.get("Origin"))
        self.set_header("Access-Control-Allow-Credentials", "true")
        ...
        return
class FileChangeAPIHandler(WebRequest):
    def post(self):
        pass
class FileAddAPIHandler(WebRequest):
    def post(self):
        if not self.current_user:
            self.finish({"info": "error","about": "not login"})
            return
        ...
        if md5 not in files:
            ...
            update_aim(block_id,block)
        self.finish({"info":"ok","about":"insert success","md5":md5})
class FileDelAPIHandler(WebRequest):
    def post(self):
        pass
