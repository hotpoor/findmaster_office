...
(function() {
  ...
  $(function() {
    ...
    root.update_search = function(_type, _search, _block_id) {
      return $.ajax({
        url: "/api/search/add",
        data: {
          block_id: BLOCK_ID,
    ...
    root.update_search_doms = function() {
      return update_search("doms", $(".sections").text().replace(/\n/g, "").replace(/\r/g, ""), BLOCK_ID);
    };
    ...
    update_dom_file = function(dom, adata) {
      var dom_content, dom_owner, dom_tree, dom_tree_list;
      dom_tree = dom.attr("data-tree");
      dom_tree_list = dom_tree.split("_");
      dom_owner = dom_tree_list[dom_tree_list.length - 1];
    ...
    root.save_dom_current = function(callback) {
      var dom_owner, dom_scroll_auto_now, dom_scroll_relative_now;
      if (callback == null) {
        callback = null;
      }
      dom_current = $(".dom_current").val();
    ...
    $("body").on("click", ".block_hollows_tool>.add", function(evt) {
      var aim_hollow_id;
    ...
    $("body").on("click", ".block_hollows_tool>.del", function(evt) {
      var aim_hollow_id;
      aim_hollow_id = $(".block_hollows_tool>.block_hollows_search").val();
      return $.ajax({
    ...
    $("body").on("click", ".section>div>.file_save", function(evt) {
      var dom, section;
      if (!send_data_now) {
        send_data_now = true;
        section = $(this).parents(".section");
        ...
        return update_dom_file_timeout = setTimeout(function() {
          return update_dom_file(dom, send_data);
        }, 500);
      }
    });
    ...
    update_headimgurl = function(adata) {
      var dom_content;
      adata["uuid"] = uuid2(6, null);
      uuid2s.push(adata["uuid"]);
      if (typeof adata["text"] === "undefined") {
      ...
      return $.ajax({
        ...
        success: function(data) {
          ...
          if (data.info === "ok") {
            return setTimeout(function() {
              return $(".block_headimgurl_save").text("save");
            }, 1000);
          }
        },
        error: function(data) {
    ...
    $("body").on("click", ".card>.card_scroll_relative", function(evt) {
      var card, card_zIndex, dom, dom_owner, dom_tree, dom_tree_list, easy_left, easy_top, max_zIndx;
    ...
    $("body").on("click", ".block_headimgurl_save", function(evt) {
      var dom, headimgurl;
      $(this).text("saving");
      ...
      return update_headimgurl_timeout = setTimeout(function() {
        return update_headimgurl(send_data);
      }, 500);
    });
    ...
    $("body").on("click", ".card>.card_more_select", function(evt) {
      ...
      if (dom.hasClass("card_more_selected")) {
        ...
        return $(".terminal_console").val((card.attr("id")) + "," + (card.position().left) + "," + (card.position().top) + "," + (card.width()) + "," + (card.height()));
      }
    });
    ...
    $("body").on("keyup", ".terminal_console", function(evt) {
      var dom_id, e, h, ref, terminal_console_action, terminal_console_action_error, terminal_console_action_list, updates, uuid_now, w, x, y;
    ...
    $("body").on("click", ".page_agora_btn.leave_room", function(evt) {
      return $("#leave").click();
    });
    ...
    return $(window).on("click", function(evt) {
      if (evt.altKey === true) {
        ...
        return root.window_alt_x_y = [evt.offsetX, evt.offsetY];
      }
    });
  });
