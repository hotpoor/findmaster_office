...
$ ->
    ...
    root.update_search = (_type,_search,_block_id)->
        $.ajax
            ...
            error:(data)->
                console.log data
    root.update_search_doms = ()->
        update_search "doms",$(".sections").text().replace(/\n/g,"").replace(/\r/g,""),BLOCK_ID
    ...
    $("body").on "click",".comment_entity_line_tools>.load_card",(evt)->
        ...
        if evt.altKey == true
            _w = parseInt(window_alt_x_y[0]-$(".main_area").offset()["left"])
            _h = parseInt(window_alt_x_y[1])
    ...
    root.save_dom_current = (callback=null)->
        ...
        dom_scroll_relative_now = ""
        if $("##{dom_current}").hasClass("dom_scroll_relative")
            dom_scroll_relative_now = "dom_scroll_relative"
        $.ajax
            ...
            data:
                ...
                dom_scroll_relative:dom_scroll_relative_now
            dataType: 'json'
            type: 'POST'
            success:(data)->
                console.log JSON.stringify(data)
    ...
    $("body").on "click",".block_editors_tool>.del",(evt)->
        ...
        $.ajax
            ...
            error:(data)->
                console.log data
    $("body").on "click",".block_hollows_tool>.add",(evt)->
        aim_hollow_id = $(".block_hollows_tool>.block_hollows_search").val()
        $.ajax
            url:"/api/page/add_hollow"
            data:
                block_id: BLOCK_ID
            ...
            error:(data)->
                console.log data
    $("body").on "click",".block_hollows_tool>.del",(evt)->
        aim_hollow_id = $(".block_hollows_tool>.block_hollows_search").val()
        $.ajax
            url:"/api/page/del_hollow"
            data:
                block_id: BLOCK_ID
                aim_hollow_id: aim_hollow_id
            dataType: 'json'
    ...
    $("body").on "click",".section>div>.video_save",(evt)->
        if not send_data_now
            ...
            update_dom_video_timeout = setTimeout ()->
                ...
                ,500
    $("body").on "click",".section>div>.file_save",(evt)->
        if not send_data_now
            send_data_now = true
            section = $(this).parents(".section")
            dom = $(this).parents(".dom")
            send_data["dom_id"]=dom.attr("id")
            send_data["text"]=
                "src":section.find(".file_src").val()
                "type":section.find(".file_type").val()
                "poster":section.find(".file_poster").val()
                "name":section.find(".file_name").val()
                "size":section.find(".file_size").val()
            editing_now(dom)
            if editing_now_timeout != null
                clearTimeout update_dom_file_timeout
            update_dom_file_timeout = setTimeout ()->
                    update_dom_file(dom,send_data)
                ,500
    ...
    update_headimgurl = (adata)->
        ...
        send_data_now = false
        $.ajax
            url:"/api/page/update_headimgurl"
            data:
                block_id: BLOCK_ID
                dom_content: dom_content
            dataType: 'json'
            ...
            success:(data)->
                ...
                if data.info =="ok"
                    setTimeout ()->
                            $(".block_headimgurl_save").text "save"
                        ,1000
            error:(data)->
                console.log data
    update_desc = (adata)->
    ...
    $("body").on "click",".card>.card_scroll_auto",(evt)->
        ...
        page_edit_current(card,true)
    $("body").on "click",".card>.card_scroll_relative",(evt)->
        console.log "card_scroll_auto update card_scroll_relative"
        dom = $(this)
        card = dom.parents(".card")
        dom_tree = card.attr("data-tree")
        dom_tree_list = dom_tree.split("_")
        ...
        dom_current = dom_tree
        if $("##{dom_current}").hasClass("dom_scroll_relative")
            card.removeClass("dom_scroll_relative")
            card_zIndex = card.css("zIndex")
            max_zIndx = get_max_zIndex()+1
            $("input.dom_position_z").val(max_zIndx)
            card.find(".z_index_num").text card_zIndex
            easy_top = parseInt(card.offset()["top"])-10
            easy_left = parseInt(card.offset()["left"]-$(".main_area").offset()["left"])+10
            card.css
                "width": "160px"
                "height": "60px"
                "padding": "0px"
            ...
            card.addClass("dom_scroll_relative")
            card.css
    ...
    $("body").on "click",".block_desc_content_save",(evt)->
        ...
        update_desc_timeout = setTimeout ()->
            ...
            ,500
    $("body").on "click",".block_headimgurl_save",(evt)->
        $(this).text "saving"
        headimgurl = $(".block_headimgurl_input").val()
        send_data["headimgurl"] = headimgurl
        dom = $(this)
        editing_now(dom)
        if editing_now_timeout != null
            clearTimeout update_headimgurl_timeout
        update_headimgurl_timeout = setTimeout ()->
                update_headimgurl(send_data)
            ,500
    ...
    $("body").on "click",".card>.card_more_select",(evt)->
        ...
        if dom.hasClass("card_more_selected")
            ...
            $(".terminal_console").val("#{card.attr("id")},#{card.position().left},#{card.position().top},#{card.width()},#{card.height()}")
        # dom_id = card.attr("id")
        # current_copy = "COPYDOM//#{BLOCK_ID},#{dom_id}"
        # if dom.hasClass("card_copy_select")
        #     dom.removeClass("card_copy_select")
    ...
    $("body").on "keyup",".terminal_console",(evt)->
        if evt.keyCode in [13] and evt.ctrlKey == true
            ...
            if terminal_console_action_list.length == 5
                ...
                try
                    ...
                    terminal_console_action_error = true 
                try
                    w = parseInt(terminal_console_action_list[3])
                catch e
                    terminal_console_action_error = true 
                try
                    h = parseInt(terminal_console_action_list[4])
                catch e
                    terminal_console_action_error = true 
    ...
    $("body").on "click",".page_agora_btn.leave_room",(evt)->
        $("#leave").click()
    root.window_alt_x_y = [null,null]
    $(window).on "click",(evt)->
        if evt.altKey == true
            console.log "window click evt",evt
            root.window_alt_x_y = [evt.offsetX,evt.offsetY]
