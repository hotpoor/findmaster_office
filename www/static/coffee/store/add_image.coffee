...
$ ->
    ...
    if window.File and window.FileList and window.FileReader and window.Blob and window.Worker
        handleFileSelect = (evt) ->
            img_add = $(this).attr("id")=="msh_hollow_img_add_upload" ? true : false
            _room_id = BLOCK_ID
        ...
        $("body").on "click",".comment_submit_add_img_more_img,.comment_submit_add_img",(evt)->
            obj = document.getElementById('msh_hollow_img_add_upload')
            obj.outerHTML = obj.outerHTML
            dom_img = $(this)
            current_dom_img = dom_img
            console.log "click to add img"
            $("#msh_hollow_img_add_upload").click()
