            ...
            <div>
                ...
                <div class="wjd_data_submit_data_k_v">
                    <div class="wjd_data_submit_data_k" data-key="瓦斯含量（%）CO2">
                        瓦斯含量（%）
                        <span class="wjd_data_submit_data_k_about">CO<sub>2</sub></span>
                    </div>
                    <div>
        ...
        <div class="wjd_data_view_area" style="display:none;">
            <div class="has_data_date_list_info">
                ...
                <div class="has_data_date_list_info_line">
                    ...
                    <div class="has_data_date_str hide">日期: <select class="has_data_date_select"></select></div>
            ...
            <div class="has_data_date_list_tools">
                ...
                <button class="need_to_print_type_add">添加一行</button>
                <button class="need_to_print_type_save">保存</button>
            </div>
            <div class="has_data_date_list_top">
                <table class="top_table">
                    <tr>
                        <th colspan="14">月度风量情况表</th>
                        <th rowspan="4" class="top_table_control">操作</th>
                    </tr>
                    <tr>
                        <th colspan="12" style="text-align:left;">单位: <span class="top_table_company"></span></th>
                        <th colspan="2" style="text-align:right;">年月: <span class="top_table_year_month"></span></th>
                    </tr>
                    <tr>
                        <th rowspan="2">进风井</th>
                        <th rowspan="2">风量<br>m<sup>3</sup>/min</th>
                        <th rowspan="2">气压<br>Mpa</th>
                        <th rowspan="2">回风井（专兼）</th>
                        <th rowspan="2">风量<br>m<sup>3</sup>/min</th>
                        <th rowspan="2">等积孔<br>m<sup>2</sup></th>
                        <th rowspan="2">负压<br>pa</th>
                        <th rowspan="2">气压<br>Mpa</th>
                        <th rowspan="2">主通型号</th>
                        <th colspan="2">运行角度</th>
                        <th rowspan="2">电机功率KW</th>
                        <th rowspan="2">机械效率</th>
                        <th rowspan="2">外漏风率</th>
                    </tr>
                    <tr>
                        <th>I</th>
                        <th>II</th>
            ...
            <div class="has_data_date_list_sign" style="margin:10px 0px;">
                <div style="display:flex;">
                    <div style="flex:1;text-align:left;">矿长（经理）：</div>
                    <div style="flex:1;text-align:left;">总工程师：</div>
                    <div style="flex:1;text-align:left;">通风区长：</div>
                    <div style="flex:1;text-align:left;">技术主管：</div>
                    <div style="flex:1;text-align:left;">填报人：</div>
                    <div style="flex:1;text-align:left;">填报日期：<span class="has_data_date_last_day"></span></div>
                </div>
                <div>
...
$("body").on "click",".need_to_print_type_save",(evt)->
    _table_tr_data = []
    _table_tr = $(".top_table_tr")
    for _tr in _table_tr
        _tds = $(_tr).find("td")
        _table_td_data = []
        for i in [0.._tds.length-2]
            _table_td_data.push $(_tds[i]).text()
        _table_tr_data.push _table_td_data
    _submit_data =
        "table":_table_tr_data
        "data_type":"top_table"
        "updatetime":(new Date()).getTime()/1000.0
    wjd_data_date_comment_submit $(".has_data_date_page_id").text(),$(".has_data_date_daily_comment_entity").attr("data-daily-chat"),JSON.stringify(_submit_data),(data)->
        if data.info == "ok"
            alert "保存成功"

$("body").on "click",".need_to_print_type_add",(evt)->
    $(".top_table").append """
    <tr class="top_table_tr">
        ...
        <td contenteditable="true"></td>
        <td contenteditable="true"></td>
        <td class="top_table_control"><button class="top_table_control_del">DEL</button></td>
    </tr>
    """
$("body").on "click",".top_table_control_del",(evt)->
    dom = $(this)
    dom.parents("tr").first().remove()

$("body").on "click",".need_to_print_type",(evt)->
    $(".top_table_control").hide()
    $(".wjd_tools").hide()
...
$("body").on "click",".wjd_plan_line_tool_submit",(evt)->
    ...
    for _wjd_item in wjd_plan_list_load_json[_data_comment_id]["json"]["wjd_items"]
        ...
        for _wjd_item_pointer in _wjd_item["tds_data"]
            _wjd_item_pointer_value = "#{_wjd_item_table}_@@_#{_wjd_item_pointer[0]}_@@_#{_wjd_item_pointer[1]}_@@_#{_wjd_item_pointer[_s_index]}"
            $(".wjd_data_pointer").append """
...
root.top_table_data_load = false
...
$("body").on "click",".wjd_plan_line_tool_view",(evt)->
    ...
    for i in [1.._date_all]
        $(".has_data_date_select").append """
            <option value="#{i}">#{i}</option>
        """
    root.has_data_date_select(_date_all)
    $(".has_data_date_last_day").text "#{_wjd_year_str}年#{_wjd_month_str}月#{_date_all}日"
...
root.wjd_data_date_load_view_all_table_json_load = (sort_type="sort_time_point_num_up",ctv=null)->
    ...
    if sort_type in ["sort_time_point_num"]
        ...
        if _d_str in ["上旬"]
            ...
            _d_str_now = "#{_y}年#{_m}月"

        $(".top_table_company").text(_company)
        $(".top_table_year_month").text(_d_str_now)
        ...
        $(".has_data_date_list").append """
            <table class="test_table">
                <tr>
                    ...
                    <th>日</th>
                    <th>S m<sup>2</sup></th>
                    <th class="hide">实际风速m/s</th>
                    <th>m<sup>3</sup>/min</th>
                    <th>°C</th>
                    <th>CH<sub>4</sub>%</th>
                    <th class="hide">瓦斯含量（%）CO2</th>
                    <th class="hide">提交时间</th>
                    <th class="hide">测风员</th>
                </tr>
            </table>
            """
        for item in _data_now
            ...
            $(".test_table").append """
                <tr>
                    ...
                    <td>#{_t}</td>
                    <td>#{_data["断面m2"]}</td>
                    <td class="hide">#{_data["实际风速m/s"]}</td>
                    <td>#{_data["实际风量m3/min"]}</td>
                    <td>#{_data["温度°C"]}</td>
                    <td>#{_data["瓦斯含量（%）CH4"]}</td>
                    <td class="hide">#{_data["瓦斯含量（%）CO2"]}</td>
                    <td class="hide">#{_t_s}</td>
                    <td class="person_select_item hide" data-name="#{_name}">#{_name}</td>
                </tr>
            """
    $(".person_select").empty()
            ...
            if data.info == "error"
                ...
                for comment in data.comments by -1
                    ...
                    if comment_json!=null
                        ...
                        if root.top_table_data_load == false
                            if comment_json["data_type"]=="top_table"
                                root.top_table_data_load=true
                                for _item in comment_json["table"]
                                    _item_tr_td_html = ""
                                    for _item_td in _item
                                        _item_tr_td_html = """
                                        #{_item_tr_td_html}
                                        <td contenteditable="true">#{_item_td}</td>
                                        """
                                    $(".top_table").append """
                                    <tr class="top_table_tr">
                                        #{_item_tr_td_html}
...
$("body").on "change",".has_data_date_select",(evt)->
    root.has_data_date_select()
root.has_data_date_select=(_wjd_date=null)->

    _data_comment_id = current_plan_json["data_comment_id"]
    if _wjd_date !=null
        $(".has_data_date_select").val(_wjd_date)
    else
...
$("body").on "click",".wjd_data_submit_data_v_calc",(evt)->
    e_vb = $(".wjd_data_submit_data_v[data-key=\"测量表风速1\"]").val()+$(".wjd_data_submit_data_v[data-key=\"测量表风速2\"]").val()+$(".wjd_data_submit_data_v[data-key=\"测量表风速3\"]").val()
    a_vb = e_vb/3.0
    a = $(".wjd_data_submit_data_v[data-key=\"风速系数a\"]").val()
    b = $(".wjd_data_submit_data_v[data-key=\"风速系数b\"]").val()
    localStorage.setItem("风速系数a", a)
    localStorage.setItem("风速系数b", b)

    vn=a*a_vb+b
    s = $(".wjd_data_submit_data_v[data-key=\"断面m2\"]").val()
    q=vn*(s-0.4)
