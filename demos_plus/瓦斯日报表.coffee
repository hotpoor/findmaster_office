...
$(window).on "load",(evt)->
    ...
    <div class="wjd_plan_edit_area" style="display:none;">
        ...
        <div class="wjd_data_submit_area" style="display:none;">
            ...
            <div>日报选择: <select class="wjd_date_submit"></select></div>
            <div>日报存储区块: <span class="wjd_date_comment_entity_submit">获取中</span></div>
            <div>
        ...
        <div class="wjd_data_view_area" style="display:none;">
            ...
            <div class="has_data_date_list_tools">
                <button class="sort_time_point_num">按点号顺序-日报表</button>
            ...
            <div class="has_data_date_list_sign" style="margin:10px 0px;">
                <div style="display:flex;">
                    ...
                    <div style="flex:1;text-align:left;">通防科长：</div>
                    <div style="flex:1;text-align:left;">通风区(队)长：</div>
                    <div style="flex:1;text-align:left;">填报单位：</div>
                </div>
                <div>
                    ...
                    <div class="wjd_plan_line" data-comment-id="#{_comment_id}_#{_comment_sequence}">
                        ...
                        <div class="wjd_plan_line_tools" style="display:flex;">
                            <div style="flex:1;display:none;">
                                <div class="wjd_plan_line_tool_submit">日报填写</div>
                            </div>
                            <div style="flex:1;">
...
root.wjd_data_date_load_view_all_table_json_load = (sort_type="sort_time_point_num",ctv=null)->
    $(".has_data_date_list").empty()
    ...
    if sort_type in ["sort_time_point_num"]
        ...
        if items_info["class_type_max"]==4
            ...
            _first_line_tr = """
                <th rowspan="3">点号</th>
                <th rowspan="3">点名</th>
                <th colspan="#{_times*9}">早班</th>
                <th colspan="#{_times*9}">中班</th>
                <th colspan="#{_times*9}">晚班</th>
                <th colspan="#{_times*9}">夜班</th>
            """
            _top_line_tr_line = """
            <tr>
                <th style="font-size:26px;" colspan="#{_times*9*4+2}">#{$(".has_data_date_company").text()}瓦斯日报表</th>
            </tr>
            <tr>
                <th style="text-align:right;" colspan="#{_times*9*4+2}">#{$(".has_data_date_year").text()}年#{$(".has_data_date_month").text()}月#{$(".has_data_date_select").val()}日</th>
            </tr>
            """
            for i in [1..4]
                for j in [1.._times]
            ...
            _top_line_tr_line = """
            <tr>
                <th style="font-size:26px;" colspan="#{_times*9*3+2}">#{$(".has_data_date_company").text()}瓦斯日报表</th>
            </tr>
            <tr>
                <th style="text-align:right;" colspan="#{_times*9*3+2}">#{$(".has_data_date_year").text()}年#{$(".has_data_date_month").text()}月#{$(".has_data_date_select").val()}日</th>
            </tr>
            """
            for i in [1..3]
                for j in [1.._times]
                ...
                for _i in _i_list
                    for j in [1..items_info["times_max"]]
                        ...
                        if _items[_i][j][i] in [undefined,null,""]
                            ...
                            item = _items[_i][j][i]
                            _t = formatDateHHMM(item[4]["wjd_data_time_date_timer"]*1000)
                            _t_s = formatDateAll(item[2]*1000)
                            _data = {}
                            for _d in item[4]["collect_data"]
                                _data[_d[0]]=_d[1]
